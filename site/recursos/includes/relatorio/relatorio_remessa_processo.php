...
//        Image(caminho, espaco a esquerda, espaco a cima, largura, altura)
            ...
            $this->Cell(0, 0, utf8_decode("RELAÇÃO DE REMESSA DE PROCESSO "), 3, 0, 'C');
            $this->Ln(5);
            $this->Cell(0, 0, "", 1, 0, 'L');
            // Line break
            $this->Ln(8);
            ...
            $headertabela = array('TIPO', 'NÚMERO', 'ANO', 'DT_CARGA', 'ASSUNTO', 'REQUERENTE', 'RECEBIDO');
            // ALIMENTO O CABECALHO DA PAGINA
            $w = array(38, 20, 15, 25, 75, 75, 63);
...
// CRIEI A FUNÇÃO PARA ALIMETAR AS LINHA E DEIXAR COR SIM COR NÃO
        function parte2() {
            ...
            if (isset($_POST['txt_op'])) {
                $id_carga = $_POST['txt_op'];
                $tamanho = count($id_carga);
                $linha = false;
