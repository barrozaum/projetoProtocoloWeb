$(function () {
    ...
    $(document).on('click', '#id_buscar_reu', function (e) {
//limpo mensagem de erro
        $("#msg_erro").html('');

//        carrego paramtros do formulario
        var reu_processo = $('#id_reu_processo').val();
        var dt_inicial = $('#id_dt_inicial').val();
        var dt_final = $('#id_dt_final').val();


// valido o requerente
        if (reu_processo.length < 3) {
            $("#msg_erro").html('<div class="alert alert-danger">POR FAVOR PREENCHA O RÉU DO PROCESSO CORRETAMENTE !! </div>');
            return false;
        }

//passo o parametro pra onde deve ir buscar
        var url = 'recursos_juridico/includes/consulta/consulta_reu_processo.php'
        var parametros = {reu_processo: reu_processo, dt_inicial: dt_inicial, dt_final: dt_final};
        var listar = "listar";
